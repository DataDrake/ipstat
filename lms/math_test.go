...
func TestLMS1(t *testing.T) {
	...
	m, b := LMS(size, rtt)
	if !math.IsNaN(m) {
		t.Error("Slope should be NaN")
	}
	if !math.IsNaN(b) {
		t.Error("Intercept should be NaN")
	}
}
...
func TestLMS2(t *testing.T) {
	...
	m, b := LMS(size, rtt)
	if math.IsNaN(m) {
		t.Errorf("Slope should not be NaN")
	}
	if math.IsNaN(b) {
		t.Errorf("Intercept should be not be Na")
	}
...
func TestLMS_Perf(t *testing.T) {
	...
	if len(rtts) != 50*15 {
		t.Error("Did not finish collecting samples")
	}
